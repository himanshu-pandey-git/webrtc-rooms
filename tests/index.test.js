...
await test('initial state is "connecting"', async () => {
  const peer = new Peer({ id: 'p1', socket: { readyState: 1, on: () => {} } });
...
await test('setMetadata removes keys set to null', async () => {
  const peer = new Peer({ id: 'p3', socket: { readyState: 1, on: () => {} } });
...
await test('send queues messages while state is RECONNECTING', async () => {
  ...
  const peer = new Peer({
    id: 'p4',
    socket: { readyState: 1, on: () => {}, send: (d) => sent.push(JSON.parse(d)) },
  });
...
await test('reconnectToken is null when reconnectTtl is 0', async () => {
  const peer = new Peer({ id: 'p6', socket: { readyState: 1, on: () => {} }, reconnectTtl: 0 });
...
await test('broadcast delivers to all peers except excluded', async () => {
  ...
  room.addPeer(a); room.addPeer(b); room.addPeer(c);

  const before = b.sent.length;
  room.broadcast({ type: 'ping' }, { exclude: 'a' });

  assert.strictEqual(a.sent.filter((m) => m.type === 'ping').length, 0, 'excluded peer received message');
  assert.strictEqual(b.sent.slice(before).filter((m) => m.type === 'ping').length, 1);
...
await test('setMetadata updates metadata and broadcasts room:updated', async () => {
  ...
  const updates = peer.sent.slice(before).filter((m) => m.type === 'room:updated');
...
const PORT = 13700;
const server = createServer({ port: PORT, autoCreateRooms: true, reconnectTtl: 3000 });
...
await test('second peer receives room:joined with first peer in list', async () => {
  ...
  assert.strictEqual(msg.peers.length, 1, 'second peer should see one existing peer');
  wsA.close(); wsB.close();
});
...
await test('first peer receives peer:joined when second peer joins', async () => {
  ...
  assert.ok(announced.peer.id, 'peer:joined should include the new peer object');
  wsA.close(); wsB.close();
});
...
await test('beforeJoin hook allows peers returning true', async () => {
  const authServer = createServer({
    port: PORT + 2,
    beforeJoin: async (peer) => peer.metadata.token === 'valid' ? true : 'denied',
  });
  await new Promise((r) => authServer.on('listening', r));

  const { ws } = await connect(PORT + 2);
  send(ws, { type: 'join', roomId: 'private-2', metadata: { token: 'valid' } });
  const msg = await waitFor(ws, (m) => m.type === 'room:joined');
...
await test('reconnect token is issued in room:joined', async () => {
  ...
  assert.ok(typeof joined.reconnectToken === 'string', 'reconnect token must be a string');
  ws.terminate();
  await sleep(100);
});
...
await test('blocks connections after maxConnPerMin is exceeded', async () => {
  ...
  assert.ok(limiter._allowConnection('10.0.0.1'));
  assert.ok(limiter._allowConnection('10.0.0.1'));
  assert.strictEqual(limiter._allowConnection('10.0.0.1'), false, '4th connection should be blocked');
  assert.strictEqual(limiter.bans().length, 1);
  limiter.destroy();
});

await test('whitelist IPs bypass all connection limits', async () => {
  const limiter = new RateLimiter({ maxConnPerMin: 1, whitelist: ['127.0.0.1'] });
  for (let i = 0; i < 20; i++) {
    assert.ok(limiter._allowConnection('127.0.0.1'), `attempt ${i + 1} should pass`);
  }
  limiter.destroy();
});
...
await test('_allowSignal blocks after maxMsgPerSec is exceeded', async () => {
  ...
  assert.ok(limiter._allowSignal('peer-1', ''));
  assert.ok(limiter._allowSignal('peer-1', ''));
  assert.strictEqual(limiter._allowSignal('peer-1', ''), false, '4th signal should be blocked');
  limiter.destroy();
});
...
await test('different peers have independent rate windows', async () => {
  ...
  assert.ok(limiter._allowSignal('peer-b', ''), 'peer-b should not be affected');
  limiter.destroy();
});
...
await test('POST /admin/rooms creates a room with metadata', async () => {
  ...
  assert.ok(server.getRoom('admin-room-1'), 'room should exist on the server');
});
...
await test('POST /admin/rooms/:roomId/broadcast delivers payload to room', async () => {
  ...
  const dataP = waitFor(ws, (m) => m.type === 'data' && m.from === '__admin__');
  const { status } = await adminReq('POST', '/rooms/broadcast-room/broadcast', {
