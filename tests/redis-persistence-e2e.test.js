...
(async () => {
  ...
  class MockRedis extends EventEmitter {
    ...
    async hSet(key, fieldOrObject, value) {
      if (!this._store.has(key))
        this._store.set(key, { type: "hash", data: new Map() });
    ...
    async sAdd(key, member) {
      if (!this._store.has(key))
        this._store.set(key, { type: "set", data: new Set() });
  ...
  await test("init() subscribes to the bus channel", async () => {
    ...
    assert.ok(
      sub._subs.has("webrtc-rooms:bus"),
      "should have subscribed to the bus channel",
    );
    await adapter.close();
  });
  ...
  await test("_onRedisMessage ignores messages from own process", async () => {
    ...
    assert.strictEqual(
      received,
      false,
      "should not emit message:received for own messages",
    );
    await adapter.close();
  });
  ...
  await test("_onRedisMessage emits message:received for foreign messages", async () => {
    ...
    adapter.on("message:received", (env) => {
  ...
  await test("getRoomPeers returns peer IDs from Redis hash", async () => {
    ...
    await pub.hSet(
      "webrtc-rooms:room:r1",
      "peer-a",
      JSON.stringify({ processId: "p1", joinedAt: 1 }),
    );
    await pub.hSet(
      "webrtc-rooms:room:r1",
      "peer-b",
      JSON.stringify({ processId: "p2", joinedAt: 2 }),
    );
  ...
  await test("throws if server is missing", async () => {
    assert.throws(
      () => new RoomPersistence({ redis: new MockRedis() }),
      /server.*required/i,
    );
  });
  ...
  await test("restore() recreates rooms from Redis snapshots", async () => {
    ...
    await redis.hSet("webrtc-rooms:snapshot:room-1", {
      metadata: JSON.stringify({ topic: "Standup" }),
      maxPeers: "10",
      createdAt: "1000000",
      savedAt: "1000001",
    });
    await redis.hSet("webrtc-rooms:snapshot:room-2", {
      metadata: JSON.stringify({ topic: "Design" }),
      maxPeers: "5",
      createdAt: "2000000",
      savedAt: "2000001",
    });
  ...
  await test("restore() skips rooms that already exist on the server", async () => {
    ...
    await redis.hSet("webrtc-rooms:snapshot:existing", {
      metadata: "{}",
      maxPeers: "50",
      createdAt: "1",
      savedAt: "2",
    ...
    assert.strictEqual(
      createCalled,
      false,
      "createRoom should not be called for existing rooms",
    );
  ...
  await test("attach() deletes snapshot when room is destroyed", async () => {
    ...
    await redis.hSet("webrtc-rooms:snapshot:dying-room", {
      metadata: "{}",
      maxPeers: "50",
      createdAt: "1",
      savedAt: "2",
  ...
  await test("listSnapshots() returns all snapshots from Redis", async () => {
    const redis = new MockRedis();
    await redis.hSet("webrtc-rooms:snapshot:r1", {
      metadata: JSON.stringify({ x: 1 }),
      maxPeers: "50",
      createdAt: "100",
      savedAt: "200",
    });
    await redis.hSet("webrtc-rooms:snapshot:r2", {
      metadata: JSON.stringify({ x: 2 }),
      maxPeers: "10",
      createdAt: "300",
      savedAt: "400",
    });
    ...
    const p = new RoomPersistence({
      redis,
      server: { on: () => {}, rooms: new Map() },
    });
  ...
  const VALID_KEY_P256 =
    "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEValidKeyBase64PaddedToCorrectLength==";
  ...
  await test("key:rotate replaces key and increments version", async () => {
    ...
    const announcedVersion = e2e.getPeerKey(
      "rotate-room",
      "peer-rotate",
    ).version;
    ...
    const confirm = peer.sent.find(
      (m) => m.type === "e2e:key:confirmed" && m.action === "rotated",
    );
  ...
  await test("key:revoked is broadcast and key is removed when peer leaves", async () => {
    ...
    assert.strictEqual(
      e2e.getPeerKey("revoke-room", "peer-revoke-a"),
      undefined,
  ...
  await test("key:request returns stored key to requesting peer", async () => {
    ...
    room.emit("data", peerA, null, {
      __e2e: "key:announce",
      publicKey: "requestableKey==",
      curve: "P-256",
    });
    await sleep(10);

    room.emit("data", peerB, null, {
      __e2e: "key:request",
      targetPeerId: "peer-req-a",
    });
    await sleep(10);
