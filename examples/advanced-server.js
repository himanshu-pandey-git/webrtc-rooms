...
const TOKEN_DB = new Map([
	['secret',       { userId: 'user-1', displayName: 'Himanshu',   role: 'admin'  }],
	['secret2',      { userId: 'user-2', displayName: 'Piysuh',     role: 'member' }],
	['secret3',      { userId: 'user-3', displayName: 'Sparsh', role: 'member' }],
	['guest-token',  { userId: 'guest',  displayName: 'Atul',   role: 'viewer' }],
]);
...
limiter.on('ip:banned', ({ ip }) => console.warn(`[security] Banned IP: ${ip}`));

// ---------------------------------------------------------------------------
// Recording adapter
// ---------------------------------------------------------------------------

const recorder = new RecordingAdapter({ outputDir: RECORD_DIR, format: 'webm' });
...
recorder.on('recording:stopped', ({ peerId, durationMs }) =>
	console.log(`[rec] Stopped  ${peerId.slice(0, 8)} (${(durationMs / 1000).toFixed(1)}s)`),
);
...
const adminApi = new AdminAPI({ server, adminSecret: ADMIN_SECRET, prefix: '/admin' });
...
server.on('peer:joined', (peer, room) => {
	...
	console.log(`[→] ${name} (${peer.metadata.role}) joined "${room.id}" (${room.size} peers)`);
});
