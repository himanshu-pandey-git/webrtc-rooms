...
server.on("listening", ({ port }) => {
  ...
  console.log(
    `Open examples/client.html in two browser tabs and join the same room.\n`,
  );
});
...
server.on("peer:joined", (peer, room) => {
  ...
  console.log(
    `[→] ${name} joined  "${room.id}"  (${room.size} peer${room.size !== 1 ? "s" : ""})`,
  );
});
...
server.on("peer:left", (peer, room) => {
  const name = peer.metadata.displayName ?? peer.id.slice(0, 8);
  console.log(
    `[←] ${name} left    "${room.id}"  (${room.size} peer${room.size !== 1 ? "s" : ""})`,
  );
});
