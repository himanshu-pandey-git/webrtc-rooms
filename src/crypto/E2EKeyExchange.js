...
class E2EKeyExchange extends EventEmitter {
  ...
  getRoomKeys(roomId) {
    ...
    if (!store) return [];
    return [...store.entries()].map(([peerId, entry]) => ({
      peerId,
      ...entry,
    }));
  }
    ...
    if (this._requireKeyOnJoin) {
      const timer = setTimeout(() => {
        ...
        if (!store?.has(peer.id)) {
          peer.send({
            type: "error",
            code: "E2E_KEY_REQUIRED",
            message: "End-to-end encryption key is required.",
          });
  ...
  _bindRoomDataEvent(room) {
    ...
    room.on("data", (fromPeer, _to, payload) => {
      ...
      this._handleE2EMessage(fromPeer, room, payload).catch((err) => {
        console.error(
          `[E2EKeyExchange] Error handling ${payload[E2E_TAG]} from "${fromPeer.id}":`,
          err.message,
        );
      });
    });
  ...
  async _handleKeyAnnounce(peer, room, payload) {
    ...
    if (store.has(peer.id)) {
      peer.send({
        type: "error",
        code: "E2E_KEY_ALREADY_ANNOUNCED",
        message: "Use key:rotate to replace an existing key.",
      });
      return;
    }
  ...
  async _handleKeyRotate(peer, room, payload) {
    ...
    if (!existing) {
      peer.send({
        type: "error",
        code: "E2E_KEY_NOT_ANNOUNCED",
        message: "Announce an initial key with key:announce first.",
      });
      return;
    }
  ...
  _validateKeyPayload(payload) {
    ...
    if (payload.publicKey.length > MAX_PUBLIC_KEY_LENGTH) {
      return {
        valid: false,
        reason: `publicKey exceeds maximum length of ${MAX_PUBLIC_KEY_LENGTH} characters.`,
      };
    }
    ...
    if (!base64Re.test(payload.publicKey)) {
      return {
        valid: false,
        reason: "publicKey must be a Base64-encoded string.",
      };
    }
